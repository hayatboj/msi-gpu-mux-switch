...
static constexpr std::array<Translation, static_cast<size_t>(Text::Count)> translations{{
    ...
    {"Intel graphics only. Lower power use for everyday work.", "Yalnızca Intel grafik birimi. Günlük işler için düşük güç tüketimi."},
    ...
    {"Experimental firmware control", "Deneysel firmware kontrolü"},
    {"Linux switching is not yet validated on real hardware. A full shutdown is required after applying a mode.", "Linux'ta geçiş henüz gerçek donanımda doğrulanmadı. Mod uygulandıktan sonra bilgisayar tamamen kapatılmalıdır."},
    ...
    {"Linux switching is experimental. A failed transition can leave a black screen and may require service recovery. A BIOS reset is not a guaranteed recovery method.", "Linux'ta geçiş deneyseldir. Başarısız bir geçiş siyah ekrana ve servisle kurtarma ihtiyacına yol açabilir. BIOS sıfırlama kesin bir kurtarma yöntemi değildir."},
